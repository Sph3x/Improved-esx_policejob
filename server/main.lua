...
RegisterServerEvent('esx_policejob:legcuff')
AddEventHandler('esx_policejob:legcuff', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'police' then
		TriggerClientEvent('esx_policejob:legcuff', target)
	else
		print(('esx_policejob: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)
...
RegisterCommand("trackOFF", function(source)
    TriggerClientEvent('trackOFF')
end, false)

RegisterCommand("trackON", function(source)
    TriggerClientEvent('trackON')
end, false)



--[[AddEventHandler('chatMessage', function(message)
    if message:sub(1) == '/trackOFF' then
        TriggerClientEvent('trackOFF')
        CancelEvent()
	end
	
	if message:sub(1) == '/trackON' then
        TriggerClientEvent('trackON')
        CancelEvent()
    end
end)]]